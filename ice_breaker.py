...
class IceBreaker:
    def __init__(self):
        self.summer_template = None

    def prompts(self):
        self.summer_template = """
        given the information {information} about a person from I want you to create:
        1. a short summary 
        2. two interesting facts about them
        """
        # To create a prompt template that will take info and templates
        summary_prompt_template = PromptTemplate(input_variables=["information"], template=self.summer_template)

        # LLM model from ChatAI
        llm = ChatOpenAI(model_name="gpt-3.5-turbo")

        # Build a chain that will run the LLM model and prompts
